...
namespace runnerDotNet
{
	...
	public class CustomExpressionProviderCS : ICustomExpressionProviderCS
	{
		...
		{
			if(table == "vt_register_doc"  &&  field == "t_register_doc_no")
			{
				var tmpVal = "REG"+DateTime.Now.ToString("yyyyMMddHHmmss");
				return XVar.Pack(tmpVal);
			}
			if(table == "dbo.t_register_doc"  &&  field == "t_register_doc_no")
			{
				var tmpVal = "REG"+DateTime.Now.ToString("yyyyMMddHHmmss");
				return XVar.Pack(tmpVal);
			}
